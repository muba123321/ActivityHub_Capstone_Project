...
//   async createActivity(req, res, next) {
//     try {
//       const { title, description, type, link, address, visibility, time, date, imageUrls } = req.body;

//       // Validate required fields based on type
//       if (type === "online" && !link) {
//         return res.status(400).json({ success: false, message: "Link is required for online activities" });
//       }

//       if (type === "physical" && !address) {
//         return res.status(400).json({ success: false, message: "Address is required for physical activities" });
//       }
...
//         address: type === "physical" ? address : undefined,
//         visibility,
//         time,
//         date,
//         imageUrls: imageUrls || ["https://picsum.photos/seed/picsum/200/300"], // default image if not provided
//         createdBy: req.user.uid, // Assuming `req.user` is set by `verifyToken` middleware
//       });
...
//         .populate("invitedUsers", "name email"); // Populate invited users' names and emails
...
//       if (existingRating) {
//         return res.status(400).json({ success: false, message: "You have already rated this activity" });
//       }
...
//           return res.status(403).json({ success: false, message: "You are not invited to this private activity" });
//         }
//       }
...
//       if (activity.createdBy.toString() !== req.user.uid) {
//         return res.status(403).json({ success: false, message: "Unauthorized to invite users to this activity" });
//       }

//       // Only for private activities
//       if (activity.visibility !== "private") {
//         return res.status(400).json({ success: false, message: "Can only invite users to private activities" });
//       }

//       // Check if user is already invited or a participant
//       if (activity.invitedUsers.includes(userId) || activity.participants.includes(userId)) {
//         return res.status(400).json({ success: false, message: "User is already invited or is a participant" });
//       }
